...
double FRoundPlanetsAsyncCallback::GravitationalConstant = 6.67430E-11;


FRoundPlanetsAsyncCallback::FRoundPlanetsAsyncCallback()
{
}
...
void FRoundPlanetsAsyncCallback::OnPreSimulate_Internal()
{
...
void FRoundPlanetsAsyncCallback::OnPreIntegrate_Internal()
{
	...
	if (FPBDRigidsSolver* MySolver = static_cast<FPBDRigidsSolver*>(GetSolver()))
	{
		...
		if (const FCustomGravityAsyncInput* Input = GetConsumerInput_Internal())
		{
			// Iterate over all the currenly simulated rigid bodies - They are named Particles in Chaos. 
			const TParticleView<FPBDRigidParticles> ActiveParticles = MySolver->GetParticles().GetNonDisabledDynamicView();
